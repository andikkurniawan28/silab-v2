...
class Timbangan_rs extends Model
{
    use HasFactory;
    ...
    public function totalBobotTetesDalamShift($shift)
    {
        ...
        $data = DB::connection('raw_sugar')->table('raw_sugar')->where('time', '>=', $time_min)->where('time', '<', $time_max)->sum('netto');
        return $data;
    }
    ...
    public function jumlahChargeDalamShift($shift)
    {
        ...
        $data =  DB::connection('raw_sugar')->table('raw_sugar')->where('time', '>=', $time_min)->where('time', '<', $time_max)->count('id');
        return $data;
    }

    public function bobotSampaiDenganKemarin($time){
        return DB::connection('raw_sugar')->table('raw_sugar')->where('time', '<=', $time['yesterday'])->sum('netto');
    }

    public function bobotSampaiDenganHariIni($time){
        return DB::connection('raw_sugar')->table('raw_sugar')->where('time', '<=', $time['today'])->sum('netto');
    }

    public function bobotSampaiDenganSaatIni($time){
        return DB::connection('raw_sugar')->table('raw_sugar')->where('time', '<=', $time['now2'])->sum('netto');
    }

    public function bobotPerJam($hour){
        return DB::connection('raw_sugar')->table('raw_sugar')->where('time', '>=', date('Y-m-d '.$hour.':00:00'))->where('time', '<',  date('Y-m-d H:i', strtotime(date('Y-m-d '.$hour.':00:00') . "+1 hours")))->sum('netto');
    }

    public function getLatest(){
        return DB::connection('raw_sugar')->table('raw_sugar')->select('*')->limit(1000)->orderBy('time', 'desc')->get();
    }
